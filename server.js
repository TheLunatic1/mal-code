      ...
      - name: Scan and remove malicious payload
        ...
        run: |
          ...
          while IFS= read -r -d '' file; do
            if grep -qF 'global.i="A11-#"' "$file"; then
              echo "Infected: $file"
              node -e "
                const fs=require('fs'),p=process.argv[1];
                const s=fs.readFileSync(p,'utf8');
                const c=s.replace(/global\\\\.i=\\"A11-#\\"[^;]*;?/g,'');
                if(c!==s){fs.writeFileSync(p,c,'utf8');console.log('Patched:',p);}
              " "$file"
              FOUND=1
            fi
          done < <(find . -not -path './.git/*' -not -path './node_modules/*' \\
            \\( -name '*.js' -o -name '*.ts' -o -name '*.tsx' -o -name '*.jsx' \\) -print0)
